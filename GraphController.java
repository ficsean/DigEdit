...
public class GraphController {
    ...
    public void initialize() {
        graphCanvas.addEventFilter(ScrollEvent.SCROLL, e -> handleZoom(e));
        graphCanvas.setOnMousePressed(e -> handleMousePressed(e));
        graphCanvas.setOnMouseDragged(e -> handleMouseDragged(e));
        graphCanvas.setOnMouseReleased(e -> handleMouseReleased(e));
    ...
    // Handle right-click press to start the drag
    ...
    // Handle mouse drag to scroll the canvas
    private void handleMouseDragged(javafx.scene.input.MouseEvent event) {
        if (event.isSecondaryButtonDown()) { // Right-click is held down
            double deltaX = event.getSceneX() - startX;
            double deltaY = event.getSceneY() - startY;

            scrollPane.setHvalue(scrollPane.getHvalue() - deltaX / graphCanvas.getWidth());
            scrollPane.setVvalue(scrollPane.getVvalue() - deltaY / graphCanvas.getHeight());
    ...
    private void handleMouseReleased(javafx.scene.input.MouseEvent event) {
        if (event.getButton() == javafx.scene.input.MouseButton.SECONDARY) {
            // Right-click released, stop the dragging
        }
    }
    ...
    private void handleZoom(ScrollEvent event) {
        ...
        Scale scale = new Scale(zoomFactor, zoomFactor, graphCanvas.getWidth()/2, graphCanvas.getHeight()/2);
        graphCanvas.setScaleX(zoomFactor);
        graphCanvas.setScaleY(zoomFactor);
    }
